<?php
    function import_sea_compute($conn, $shipment_details_refs) {
        $sql = "SELECT forwarder_name from m_shipment_sea_details where shipment_details_ref = :shipment_details_ref";
    ...
    function import_sea_table_render($conn, $computed_mega_json) {
        $sql = "SELECT charge_group, details_of_charge, currency from m_billing_information where type_of_transaction = 'IMPORT SEA' order by case charge_group when 'LOCAL CHARGES' then 1 when 'ACCESSORIAL' then 2 when 'REIMBURSEMENT' then 3 else 4 end";
        $stmt = $conn -> prepare($sql);
        $stmt -> execute();
