<?php
    function import_air_compute($conn, $shipment_details_refs) {
        ...
        foreach ($shipment_details_refs as $shipment_detail_ref) {
            ...
            $array_computation = array_fill(0, 23, 1);
    ...
    function import_air_table_render($conn, $computed_mega_json) {
        $sql = "SELECT charge_group, details_of_charge, currency from m_billing_information where type_of_transaction = 'IMPORT AIR' order by case charge_group when 'LOCAL CHARGES' then 1 when 'ACCESSORIAL' then 2 when 'REIMBURSEMENT' then 3 else 4 end";
        $stmt = $conn -> prepare($sql);
        $stmt -> execute();
