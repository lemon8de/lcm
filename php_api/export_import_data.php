...
if ($show_active == "true") {
    $sql = "SELECT shipper, origin_port, destination_port, shipping_invoice, commodity, classification, commodity_quantity, commodity_uo, commercial_invoice_currency, commercial_invoice_amount, incoterm, gross_weight, forwarder_name, bl_number, vessel_name, eta_mnl, ata_mnl, atb, container, container_size, shipping_lines, shipment_status, required_delivery_sched, deliver_plan, tabs, date_port_out, actual_received_at_falp, polytainer_size, polytainer_quantity, etd, container_status, date_return_reused, no_days_port, no_days_falp, type_of_expense, ip_number, dr_number, received_by, time_received, total_custom_value, duitable_value, rate, customs_duty, landed_cost, vat, bank_charges, wharfage, arrastre_charges, entry_no, or_number, assessment_date from import_data as a left join m_shipment_sea_details as b on a.shipment_details_ref = b.shipment_details_ref left join m_vessel_details as c on a.shipment_details_ref = c.shipment_details_ref left join m_delivery_plan as d on a.shipment_details_ref = d.shipment_details_ref left join m_completion_details as e on a.shipment_details_ref = e.shipment_details_ref left join m_polytainer_details as f on a.shipment_details_ref = f.shipment_details_ref left join m_mmsystem as g on a.shipment_details_ref = g.shipment_details_ref WHERE actual_received_at_falp BETWEEN CAST(CONCAT(:start_year, '-', :start_month, '-01') AS DATE) AND EOMONTH(CAST(CONCAT(:start_year2, '-', :start_month2, '-01') AS DATE)) ORDER BY shipping_invoice asc";
} else {
